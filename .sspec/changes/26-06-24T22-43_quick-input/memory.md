...
## State
Plan 已完成（tasks.md 5 阶段）。待 @align report 后进入 Implement。Phase 0 三个内核假设验证（V-1/V-2/V-3）为 Implement 首任务。
...
- [2026-06-24] [Gotcha] `insertBlock(dataType, data, nextID?, previousID?, parentID?)` 签名：next/prev 兄弟插入传 nextID/previousID，不传 parentID（`api.d.ts:35`）
