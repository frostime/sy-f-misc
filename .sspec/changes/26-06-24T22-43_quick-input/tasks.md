...
### Feedback Tasks (→ [001-rename-block-modes-before-after](./revisions/001-rename-block-modes-before-after.md))
- [x] 修正 block sibling mode 命名：`next/prev` → `before/after`

### Feedback Tasks (→ [002-guard-duplicate-document-hpath](./revisions/002-guard-duplicate-document-hpath.md))
- [x] `src/func/quick-input/engine.ts` document 模式创建前调用 `getIDsByHPath(notebook, hpath)` 检查重复 hpath
- [x] `engine.ts` 发现重复 hpath 时用 `confirm` 请求用户确认；取消则中止且不显示 error toast
- [x] `src/func/quick-input/panel.tsx` 捕获 `QuickInputCancelled` 并静默返回
- [x] `pnpm run type-check` + `pnpm run build` 通过

### Feedback Tasks (external review follow-up)
- [x] Accepted: `src/func/quick-input/engine.ts` 将共享 `g` regex 改为 factory，避免未来 `.test/.exec` 误用 `lastIndex`
- [x] Accepted: `src/func/quick-input/config.ts` 对 `declaredInputVar` 做字段归一化与非法项过滤
- [x] Optional semantic choice: `src/func/quick-input/setting.tsx` number 默认值清空时保留为空值
- [x] Not a bug: `InsertMode` type-only import 保持不变，仅加注释说明
- [x] `pnpm run type-check` + `pnpm run build` 通过

### Feedback Tasks (UI/UX review → [003-ui-polish-and-dailynote-syntax](./revisions/003-ui-polish-and-dailynote-syntax.md))
- [x] `src/func/quick-input/setting.tsx` 修复右侧编辑区滚动/溢出布局
- [x] `setting.tsx` 将「插入今日日记」按钮移到 block 模式区域顶部
- [x] `setting.tsx` 增加帮助文档按钮，调用 `documentDialog`
- [x] `public/docs/quick-input.md` 编写完整使用说明
- [x] `src/func/quick-input/engine.ts` 支持 `${todayDailynoteId:<notebookId>}` 内联语法
- [x] `pnpm run type-check` + `pnpm run build` 通过

### Feedback Tasks (UI/UX review 004 → [004-ui-notebook-picker-and-field-layout](./revisions/004-ui-notebook-picker-and-field-layout.md))
- [x] `src/func/quick-input/setting.tsx` 修复「插入后打开」toggle 布局错乱
- [x] `setting.tsx` 用 `lsNotebooks()` 加载笔记本列表，所有笔记本 ID 输入改为下拉选择
- [x] `setting.tsx` 「输入字段」每个字段改为独立卡片多行布局，增强视觉分区
- [x] `pnpm run type-check` + `pnpm run build` 通过

### Minor fixes (no revision)
- [x] `setting.tsx` 基础信息手写表单布局，修复「插入后打开」toggle 过长问题
- [x] `setting.tsx` 「输入字段」卡片改为 label + input 对齐的 form 样式
- [x] `setting.tsx` 「插入今日日记」按钮移到 anchorId 行尾部
- [x] `engine.ts` 修复 `${todayDailynoteId:<notebookId>}` 内联语法未被模板渲染的 bug
- [x] `src/libs/components/simple-form.tsx` 修复 checkbox 行被拉伸问题
- [x] `src/func/quick-input/types.ts` / `config.ts` / `setting.tsx` 增加模板 `description` 字段
- [x] `src/func/quick-input/panel.tsx` Alt+I 面板按钮改为 mini card，显示名称 + 说明
...
- 2026-06-25T02:17: runtime report 显示 createDocWithMd 同 hpath 会静默创建重复文档；记录 revision 002，状态回到 DOING
- 2026-06-25T02:22: revision 002 完成；重复 hpath 创建前 confirm；`pnpm run type-check` + `pnpm run build` 通过
- 2026-06-25T15:44: external review follow-up 完成；接受 regex factory 与 declaredInputVar 归一化；number 空值保留为可选语义修正；InsertMode type-only import 仅注释说明；`pnpm run type-check` + `pnpm run build` 通过
- 2026-06-25T15:49: 修正 external review 记录措辞，避免误示四项都被判定为必须修复
