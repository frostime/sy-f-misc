 ...
 * 插入位置类型
 */
type InputPlace = 'block' | 'document' | 'dailynote';
 ...
 * 新输入模板配置
 */
interface INewInputTemplate<T extends InputPlace = InputPlace> {
    /** 模板唯一标识 */
    id: string;
 ...
 * 中间变量（基础变量 + 用户输入）
 */
interface IMidVar extends IBasicVar {
 ...
 * 模板变量（完整上下文）
 */
interface ITemplateVar extends IMidVar {
    /** 所在的文档 Block 对象 */
    root: Block;
